...
def triagx(dot):
    ...
    res=q/p
    return res    
def dviangle(dot, num):
    diam=[]
    diam2=[]
    diam3=[]
    for i in range(num):
        for j in range(num):
            r=distance(dot[i][0],dot[i][1],dot[j])
            diam.append(r)
            diam2.append(i)
            diam3.append(j)     
    
    rad=diam.index(max(diam))
    x=diam2[rad]
    y=diam3[rad]
    r=distance(dot[x][0], dot[x][1], dot[y])
    return [(dot[x][0]+dot[y][0])*1/2, (dot[x][1]+dot[y][1])*1/2, r*1/2]    
